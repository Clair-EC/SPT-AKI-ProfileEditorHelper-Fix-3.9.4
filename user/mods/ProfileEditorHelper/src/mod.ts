...
class Mod implements IPostSptLoadMod {
    ...
    public postSptLoad(container: DependencyContainer): void {
        ...
        if (this.vfs.exists(this.hashesPath)) {
            const hashesObject = this.jsonUtil.deserialize(this.vfs.readFile(this.hashesPath));
            this.hashesDictionary = new Map<string, string>(Object.entries(hashesObject));
        }
        ...
        if (this.hasDataUpdates) {
            const hashesObject = Object.fromEntries(this.hashesDictionary);
            this.vfs.writeFile(this.hashesPath, this.jsonUtil.serialize(hashesObject, true));
    ...
    private exportDictionaryEntry(name: string, dictionary: object): void {
        ...
        for (const [key, value] of Object.entries(dictionary)) {
            ...
            if (this.hashesDictionary.get(entryName) === valueHash) {
                continue;
            }
            this.vfs.writeFile(`${this.exportPath}/${entryName}.json`, valueJson);
            this.hashesDictionary.set(entryName, valueHash);
